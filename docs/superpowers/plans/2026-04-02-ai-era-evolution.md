...
Run: `go build ./...`
Expected: 编译失败，有两类原因：
1. `root.go` 引用了已删除命令的 `NewCmd*()` 函数（decode、encode、time、word、sql）
2. `root.go` 的 `ViperInitSet()` 引用了 `sql.go` 中定义的 SQL 常量（`SqlConfigType` 等）

> 注：`json.go` 和 `pypi.go` 通过自身 `init()` 注册命令，删除文件即可自动清理，无需在 `root.go` 中处理。
...
删除不再使用的 import：`crypto/md5`, `crypto/sha1`, `encoding/base64`, `encoding/hex`, `net/url`, `strconv`, `fmt`。

> 注：`unicode_encode` case 使用了 `fmt.Sprintf`，删除后 `fmt` 不再被使用。检查保留的 case（`open`、`//`、`??`、`>`）和 `showConsoleHelp()` 均不使用 `fmt`。
...
将 `NewCmdHttp()` 中的 `Run` 改为 `RunE`，函数签名从 `func(cmd *cobra.Command, args []string)` 改为 `func(cmd *cobra.Command, args []string) error`。

将 `http.ListenAndServe(":"+strHttpPort, nil)` 改为 `return http.ListenAndServe(":"+strHttpPort, nil)`。
...
```bash
git add cmd/http.go
git commit -m "fix: http 命令改为 RunE + 上传安全修复（路径穿越防护 + 100MB 限制）"
```
...
> 注：`newSearchCmd()` 已经是 `RunE`，只需修改 `NewCmdBuild()` 中的 `Run` → `RunE`。

将 `NewCmdBuild()` 的 `Run` 改为 `RunE`，将 `log.Fatalln("start build fail", err.Error())` 改为 `return fmt.Errorf("start build fail: %w", err)`。
移除 `"log"` import（如果不再使用）。
